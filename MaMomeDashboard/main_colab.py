...
colabdir = "/content/drive/My Drive/MaMomeDashboard"

noart = [""]
articles = tuple(sorted(os.listdir(colabdir + "/MaMomePDFs")))
...
if bert_type == "General":
    with open(colabdir + "Summary_BL/" + summary_name, "r") as f:
        summary = f.read()
else:
    with open(colabdir + "Summary_PM/" + summary_name, "r") as f:
        summary = f.read()
...
with open(colabdir + '/docproc/bac_list.csv') as f:
    reader = csv.reader(f)
    blist = list(reader)
...
with open('Nutrient_keywords.csv') as f:
    reader = csv.reader(f)
    nlist = list(reader)
...
if show_raw:
    ...
    with open(colabdir + "/docproc/TxtData/" + raw_name) as f:
        raw = f.read()
